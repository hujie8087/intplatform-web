  ...
  <el-dialog v-model="dialogVisible" :title="`批量添加${parameter.title}`" :destroy-on-close="true" width="580px" draggable>
    <el-form class="drawer-multiColumn-form" label-width="100px">
      <el-row>
        ...
        <!-- <el-col :span="12">
          <el-form-item label="更新已经存在的用户数据 :" label-width="180px">
...
// Excel 导入模板下载
const downloadTemp = () => {
  if (!parameter.value.tempApi) return false;
  useDownload(baseUrl + parameter.value.tempApi, `${parameter.value.title}模板`, true, ".xlsx", "get");
};
...
const uploadExcel = async (param: UploadRequestOptions) => {
  ...
  excelFormData.append("projectKey", parameter.value.projectKey);
