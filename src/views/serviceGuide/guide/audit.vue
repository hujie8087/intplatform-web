              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item label="服务指南类型" prop="typeId">
                    <el-select v-model="formData.typeId" placeholder="请选择" disabled>
                      <el-option v-for="item in guideTypeOptions" :label="item.label" :value="item.value" :key="item.value">
                      </el-option>
                    </el-select>
                  </el-form-item>
                </el-col>
                ...
                <el-col :span="24">
                  <el-form-item label="排序" prop="sort">
                    <el-input-number v-model="formData.sort" :min="0" :max="9999" disabled />
                  </el-form-item>
                </el-col>
              </el-row>
            </el-col>
...
<script setup lang="ts" name="GuideAudit">
...
import { editGuide, getGuideById } from "@/api/modules/serviceGuide/guide";
...
const formData = ref<Guide.ResGuide>();
...
const handleSubmit = async () => {
  ...
  try {
    await editGuide({ ...formData.value!, nextApprovalFlag: nextApprovalFlag.value });
    ElMessage.success({
...
const handleReject = async () => {
  ...
  try {
    await editGuide({ ...formData.value!, nextApprovalFlag: 1 });
    ElMessage.success({
      message: t("main.successMsg", { title: "服务指南", method: "审核" })
    });
