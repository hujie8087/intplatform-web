<template>
  <div class="main-box">
    <div class="table-box">
      ...
      >
        ...
        <template #tableHeader="scope">
          <el-button type="primary" :icon="CirclePlus" @click="openDrawer('新增')">新增视频集</el-button>
    ...
    <VideoDetailsDrawer ref="drawerVideoRef" />
  </div>
</template>
...
import { addGather, deleteGather, editGather, getGatherList, getVideoLabelList, getVideoTypeList } from "@/api/modules/video";
import { CirclePlus, Delete, EditPen, VideoPlay } from "@element-plus/icons-vue";
import { useHandleData } from "@/hooks/useHandleData";
import GatherDrawer from "./components/GatherDrawer.vue";
import VideoDetailsDrawer from "./components/VideoDetailsDrawer.vue";
...
// 打开 drawer(新增、查看、编辑)
const drawerVideoRef = ref<InstanceType<typeof VideoDetailsDrawer> | null>(null);
const openVideoDrawer = async (row: Partial<Gather.ResGather> = {}) => {
  const params = {
    rowData: { ...row }
  };
  drawerVideoRef.value?.acceptParams(params);
};
</script>
