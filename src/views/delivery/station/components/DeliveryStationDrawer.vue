<template>
  <el-dialog v-model="drawerVisible" :destroy-on-close="true" width="800" :title="`${drawerProps.title}配送站点`">
    <el-form
      ref="ruleFormRef"
      label-width="120px"
      label-suffix=" :"
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="来源类型" prop="sourceType">
            <el-select v-model="drawerProps.rowData.sourceType" placeholder="请选择来源类型">
              <el-option
                v-for="item in drawerProps.sourceTypeOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
        ...
        <el-col :span="12">
          <el-form-item label="站点编码" prop="code">
            <el-input
              ...
              :placeholder="`${$t('main.inputError', '站点编码')}`"
              clearable
            ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="站点名称" prop="sourceStation">
            <el-input
              v-model="drawerProps.rowData.sourceStation"
              :placeholder="`${$t('main.inputError', '站点名称')}`"
              clearable
            ></el-input>
          </el-form-item>
        </el-col>
        ...
        <el-col :span="24">
          <el-form-item label="地址" prop="address">
            <el-input
              ...
              :placeholder="`${$t('main.inputError', '地址')}`"
              clearable
            ></el-input>
          </el-form-item>
        </el-col>
...
const rules = reactive({
  sourceType: [{ required: true, message: t("main.selectError", { msg: "来源类型" }) }],
  sourceStation: [{ required: true, message: t("main.inputError", { msg: "站点名称" }) }],
  code: [{ required: true, message: t("main.inputError", { msg: "站点编码" }) }],
  contacts: [{ required: true, message: t("main.inputError", { msg: "联系人" }) }],
  tel: [{ required: true, message: t("main.inputError", { msg: "联系电话" }) }],
  address: [{ required: true, message: t("main.inputError", { msg: "地址" }) }],
...
interface DrawerProps {
  title: string;
  isView: boolean;
  rowData: Partial<DeliveryStation.ResDeliveryStation>;
  api?: (params: any) => Promise<any>;
  getTableList?: () => Promise<any>;
  sourceTypeOptions?: DictOptions[];
...
const handleSubmit = () => {
  ruleFormRef.value!.validate(async valid => {
    ...
    try {
      ...
      ElMessage.success({
        message: t("main.successMsg", { title: "配送站点", method: `${drawerProps.value.title}` })
      });
