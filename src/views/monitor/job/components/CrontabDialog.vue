<template>
  <el-dialog v-model="drawerVisible" :destroy-on-close="true" width="880" title="Cron表达式生成器">
    <el-tabs type="border-card">
      <el-tab-pane label="秒"> <Second :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
      <el-tab-pane label="分钟"> <Min :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
      <el-tab-pane label="小时"><Hour :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
      <el-tab-pane label="日"><Day :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
      <el-tab-pane label="月"><Month :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
      <el-tab-pane label="周"><Weekly :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
      <el-tab-pane label="年"><Year :cron="crontabValueObj" :check="checkNumber" /></el-tab-pane>
    </el-tabs>
...
const submitFill = () => {
  drawerProps.value.crontabFill!(crontabValueString.value);
  hidePopup();
};
const hidePopup = () => {
...
const expressionChange = () => {
  ...
  let ruleArr = crontabValueString.value.split(" ");
  // 用于记录进入循环的次数
  let nums = 0;
  // 用于暂时存符号时间规则结果的数组
