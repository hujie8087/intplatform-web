...
const getOrganismTypeData = async () => {
  const res = await getOrganismTypeList({ pageNum: 1, pageSize: 9999, type: initParam.type });
  organismTypeData.value = res.rows.map(item => ({
    label: item.name,
    value: item.id
  }));
};
...
const openDrawer = async (num: number, rowData: Partial<Animals.ResAnimals> = {}) => {
  ...
  let params = {
    ...
    organismTypeOptions: organismTypeData
  };
