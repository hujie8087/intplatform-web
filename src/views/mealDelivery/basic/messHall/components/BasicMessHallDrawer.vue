  ...
  <el-dialog v-model="drawerVisible" :destroy-on-close="true" width="800" :title="`${drawerProps.title}站点`">
    ...
    >
      <el-row>
        <!-- 来源类型 -->
        <el-col :span="12">
          <el-form-item label="食堂名称" prop="fdName">
            <el-input
              v-model="drawerProps.rowData.fdName"
              :placeholder="`${$t('main.inputError', '食堂名称')}`"
              clearable
            ></el-input>
          </el-form-item>
        </el-col>
...
<script setup lang="ts" name="BasicMessHallDrawer">
...
const rules = reactive({
  fdId: [{ required: true, message: t("main.inputError", { msg: "食堂名称" }) }],
...
interface DrawerProps {
  title: string;
  isView: boolean;
  rowData: Partial<BasicMessHall.ResBasicMessHall>;
...
const handleSubmit = () => {
  ruleFormRef.value!.validate(async valid => {
    ...
    try {
      ...
      ElMessage.success({
        message: t("main.successMsg", { title: "食堂", method: `${drawerProps.value.title}` })
      });
