...
import { addMenu, delMenu, getMenu, listMenu, updateMenu } from "@/api/modules/mdc/system/menu";
...
const openDrawer = async (num: number, rowData: Partial<Menu.ResMenu> = {}) => {
  ...
  if (num === 3) {
    const { data } = await getMenu(rowData.menuId as number);
    menuInfo = data;
  } else if (num === 2) {
...
const deleteAccount = async (params: Menu.ResMenu) => {
  await useHandleData(delMenu, params.menuId, t("main.deleteMsg", { msg: params.menuName, title: t("system.menu.menu") }));
